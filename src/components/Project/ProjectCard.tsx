...
import {PiCode, PiCornersOutLight, PiFigmaLogoLight, PiGlobeHemisphereWestThin} from "react-icons/pi";
...
export const ProjectCard: FC<ProjectCardInterface> = (props: ProjectCardInterface) => {
    ...
    return (
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-5 text-neutral-300">
            {(image && screenshots) && (
                <div
                    className={classNames("relative aspect-video sm:aspect-square overflow-hidden w-full sm:w-48 md:w-52 group", {
                        "cursor-pointer": !isLoading
                    })}
                    onClick={() => !isLoading && onImagePreview(project as ProjectWithScreenshotsInterface)}
                >
                    <Image
            ...
            <div className="flex-1">
                ...
                <div className="flex flex-row flex-wrap gap-2 mt-4">
                    {
                        uses.map(item => (
                            <span className="text-xs sm:text-sm px-3 py-1.5 rounded-md bg-black select-all"
                                  key={`${item}-${title}`}>{item}</span>
                        ))
                    }
                </div>
            </div>
