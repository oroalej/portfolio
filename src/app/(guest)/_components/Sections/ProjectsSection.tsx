...
const ProjectsSection = () => {
  ...
  const [type, setType] =
    useState<PropType<ProjectInterface, "type">>("freelance");

  const [activeProject, setActiveProject] =
    useState<ProjectWithScreenshotsInterface>({
      type: "personal",
      title: "",
      description: "",
      image: "",
    ...
    <Fragment>
      <Section className="bg-neutral-900 text-neutral-300" id="projects">
        ...
        <p className="text-sm xs:text-base mb-8 text-opacity-80">
          Whatever makes me interested and curious, I will try to learn it by
          taking a freelance work using that technology or creating my own
          project just to try it out.
        </p>
        ...
        <div className="flex flex-row gap-2">
          <button
            ...
            className={classNames(
              "hover:bg-neutral-800 active:bg-neutral-800 active:bg-opacity-80 transition-colors px-4 py-2 rounded-md",
              {
        ...
        <Section.Content>
          <AnimatePresence mode="sync">
            {PROJECT_LIST.filter((item) => item.type === type).map(
              (project) => (
                <ProjectCard
