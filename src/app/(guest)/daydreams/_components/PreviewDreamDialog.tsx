...
export const PreviewDreamDialog = (
  props: Required<Omit<DialogProps, "children">>
) => {
  ...
  const { isLoading: isImageFetchLoading, data: imageUrl } =
    useStoragePublicUrl(selectedItem.file.storage_file_path);
  ...
  useEffect(() => {
    ...
    const scale =
      Number(selectedItem.file.height) /
      Number(imageContainerRef.current?.clientHeight || 1);
    ...
    <Dialog isOpen={isOpen}>
      <CardRoot rounded className="h-full max-w-[1920px] mx-auto">
        <div className="p-3.5 md:p-6 flex flex-col lg:flex-row h-full gap-2 lg:gap-8">
          ...
          <div className="relative flex justify-between items-center lg:px-3 overflow-hidden grow bg-neutral-200 rounded-md mt-12 lg:mt-0">
            {isLoading && (
          ...
          <div className="flex flex-row-reverse lg:flex-col w-full lg:w-80">
            <div className="grow text-right lg:grow-0 lg:text-left">
              ...
              <FormGroup>
                ...
                {isLoading ? (
                  <BaseSkeletonLoader style={{ height: "28px" }} />
                ) : (
