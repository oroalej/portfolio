import {ProjectContent, SectionTitle, SectionWrapper, Skill, SkillInterface} from "@/components/index";
...
const Skills = () => {
  ...
  return (
    <SectionWrapper className="text-neutral-600" id="what-i-use">
      <SectionTitle className="text-xl sm:text-2xl md:text-3xl" anchor="#what-i-use">
        Tech Stack
      </SectionTitle>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-10 md:gap-20">
          {list.map(({title, data}) => (
            <div className="block" key={title}>
              <h3 className="text-base sm:text-lg lg:text-xl font-bold capitalize mb-3 select-all">{title}</h3>

              <div className="flex flex-col gap-5 text-sm sm:text-base lg:text-lg">
                {data.map(({icon, text}) => (
                  <Skill icon={icon} text={text} key={text}/>
                ))}
              </div>
            </div>
          ))}
